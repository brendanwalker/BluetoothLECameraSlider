...
namespace CameraSlider.Bluetooth
{
  public class CameraSliderDevice
  {
    ...
    private GattCharacteristic _sliderPosCharacteristic= null;
    private GattCharacteristic _sliderSpeedCharacteristic= null;
    private GattCharacteristic _sliderAccelCharacteristic= null;
    ...
    private async Task<bool> SetupCameraSliderCharacteristics()
    {
      ...
      _sliderPosCharacteristic = await GetServiceCharacteristicByUuidAsync(_cameraSliderService, SLIDER_POS_CHARACTERISTIC_UUID);
      if (_sliderPosCharacteristic == null)
        return false;

      _sliderSpeedCharacteristic = await GetServiceCharacteristicByUuidAsync(_cameraSliderService, SLIDER_SPEED_CHARACTERISTIC_UUID);
      if (_sliderSpeedCharacteristic == null)
        return false;

      _sliderAccelCharacteristic = await GetServiceCharacteristicByUuidAsync(_cameraSliderService, SLIDER_ACCEL_CHARACTERISTIC_UUID);
      if (_sliderAccelCharacteristic == null)
        return false;
    ...
    private void CleanUpGattState()
    {
      if (_cameraSliderDevice != null)
      {
        ...
        _sliderPosCharacteristic = null;
        _sliderSpeedCharacteristic = null;
        _sliderAccelCharacteristic = null;
    ...
    private async Task<bool> SendCommand(string command)
    {
      try
      {
        byte[] Message = new byte[] { Convert.ToByte(command) };
        GattWriteResult result = await _commandCharacteristic.WriteValueWithResultAsync(Message.AsBuffer());
